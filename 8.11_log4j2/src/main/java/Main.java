...
public class Main
{
    ...
    private static final Marker RIGHT = MarkerManager.getMarker("RIGHT");
    private static final Marker WRONG = MarkerManager.getMarker("WRONG");
    ...
    private static Station takeStation(String message)
    {
        for(;;)
        {
            ...
            if(station != null) {
                logger.info(RIGHT, "Станция найдена: {}", station); //логгер для сбора сущ-х станций
                return station;
            }
            logger.info(WRONG,"Станция не найдена: {}", line); //логгер для сбора не сущ-х станций
